...
private:
	...
	///////////////////////////////////////////////////
	// <your declarations>
	// use hit/miss matrix globally
	int hits[1000][1000] = {};  //this->map.info.width*this->map.info.height = 1000*1000
...
void MappingNode::laserReceived( const sensor_msgs::LaserScanConstPtr& laserScan ) {
	...
	//iterations through all laser measurements
	for (int i = 0; i < (laserScan->angle_max - laserScan->angle_min)/laserScan->angle_increment ; i++){ 
		//if measured laser is in valid range
		if( (laserScan->range_min < laserScan->ranges[i]) && (laserScan->ranges[i] < laserScan->range_max)){ 
			vector<pair<int, int> > result;

			//calculate end of laser for bresenhamLine()
			int x_end = robotX + laserScan->ranges[i]/map.info.resolution * cos (robotTheta + laserScan->angle_min + laserScan->angle_increment*i);
			int y_end = robotY + laserScan->ranges[i]/map.info.resolution * sin (robotTheta + laserScan->angle_min + laserScan->angle_increment*i);

			result = bresenhamLine( robotX, robotY, x_end, y_end);
			double value_update = 0;

			for (int j = 0; j < result.size(); j++){
					if(j == result.size()-1 ){ // this is the position of the wall
						hits[result[j].first][result[j].second]++;
					}
					else{ // other fields should be empty
			...
			// a simple way without matrix
			// for (int j = 0; j < result.size()-1; j++){
			// 	updateReflectionMapValue(result[j].first, result[j].second , 0);
			// }
			// updateReflectionMapValue(result[result.size()-1].first, result[result.size()-1].second , 1);
